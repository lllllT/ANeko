...
public class ANekoActivity extends AppCompatActivity {
  ...
  private void init() {
    getSupportFragmentManager()
            .beginTransaction()
            .replace(R.id.neko_prefs, new SettingsFragment(ANekoActivity.this))
            .commit();
  }
      ...
      if(key != null) {
        switch (key) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
              if (!Settings.canDrawOverlays(getContext())) {
                ...
                getActivity().getSupportFragmentManager()
                        .beginTransaction()
                        .replace(R.id.neko_prefs, new SettingsFragment(getContext()))
                        .commit();
              }
            }
            break;
          ...
          case "get.skin":
            ...
            intent.setData(Uri.parse(getContext().getString(R.string.skin_search_uri)));
            try {
              getContext().startActivity(intent);
            }
            catch(ActivityNotFoundException e) {
              Toast.makeText(getContext(), R.string.msg_market_not_found, Toast.LENGTH_SHORT).show();
            }
        }
      }
